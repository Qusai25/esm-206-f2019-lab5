...
ggplot(lobster_tidy, aes(x = size_mm)) +
  ...
  facet_wrap(~site, scales = "free") # gives you different graphs instead of putting it all in one

#if you want to put each group to have seperate scales, you can do this:facet_wrap(~site, scales = "free")
...
#I am going to add a new column that contains my data as an actual date
lobster_date <- lobster_tidy %>% 
  mutate(
    date_new = lubridate::mdy(date)
  ) # this will make the date version into the version that R likes, so it is changing from monthdayyear or mdy to year month day

# also the class has changed to "date"
...
# this is adding another column for year and another column for month

# this only works because date_new is a date class. otherwise lubridate will not find it.
...
lobster_site <- lobster_parse_date %>% 
  count(obs_year, obs_month, site = "carp", size_mm)

lobster_site

#group _by + summarize +n()

lobster_summary <- lobster_parse_date %>% 
  group_by(site) %>%
  summarise(mean_size =
              mean(size_mm, na.rm = TRUE))
...
We can use the `t.test()` function to find confidence intervals and perform t-tsts. First, I'll isolate the IVEE lobster lengths and find a 95% confidence interval. 
```{r}
...
two sample t-test:
IS there a significant difference in mean lobster lengthsbetween Naples Reef and Mohalk Reef?
...
```
mean lobster size differed significantly between mohalk and Naples reefs (t(`r mohk_napl_ttest$parameter`) = 19.849)

maybe their is an easier way
```{r}
lobster_2sample <- lobster_tidy %>% 
  filter(site %in% c("NAPL", "MOHK"))
